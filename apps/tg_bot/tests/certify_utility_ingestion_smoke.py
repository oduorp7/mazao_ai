...
class TestUtilityIngestionSmoke(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_electricity_token_full_sms_ingestion(self, mock_gen_tip, mock_db):
    ...
    @patch('apps.tg_bot.handlers.db')
    @patch('apps.agent.tips.generate_tip', new_callable=AsyncMock)
    async def test_tier_c_safe_degradation(self, mock_gen_tip, mock_db):
        mock_db.get_tenant.return_value = self.tenant
        mock_db.get_conv_state.return_value = {"state": "idle"}
    ...
    async def test_gas_refill_ingestion(self, mock_get_proj, mock_db):
        mock_db.get_tenant.return_value = self.tenant
        ...
        self.update.message.text = "13 22/04/2026"
